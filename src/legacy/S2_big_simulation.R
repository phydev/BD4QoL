...
for (data_id in 1:max(df$id)) {
       ...
       imp_ranger <- miceRanger::miceRanger(
              df_m, 
              m = m_imputations, 
              maxiter=20, 
              seed=42, 
       ...
       r_ranger <- c(1:m_imputations)

       for (i in 1:m_imputations) {

              df_ranger <- miceRanger::completeData(imp_ranger, i)[[paste0("Dataset_", i)]]
              lm_s_ranger <- lm(y ~ x1 + x2, data=df_ranger) %>% summary
              r_ranger[i] <- lm_s_ranger$r.squared
       #print(lm_s$r.squared)
       
       }

       temp <- pool_r_squared(r_ranger, sample_size= nrow(df_m)) %>% as.data.frame
...
write.csv(results, paste0("r2_tuned_", data_file,".csv"),  row.names=FALSE)