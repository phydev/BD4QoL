...
df <- read.csv(paste0(path, "data/BD4QoL_150523_encoded.csv"), stringsAsFactors = TRUE)
...
df <- df %>% filter(!is.na(hn4_dv_c30_ghs))

missing <- rbinom(nrow(df), 1, 0.5)

df$missing <- as.factor(missing)

df_complete <- df

df <- df %>% mutate(hn4_dv_c30_ghs = if_else(missing==1, NA_real_, hn4_dv_c30_ghs))
...
df_imputed <- miceRanger::completeData(imp_all)
aa <- df_imputed$Dataset_1
aa$missing <- df$missing
model <- lm(hn4_dv_c30_ghs ~ hn1_na8_cb_sex + hn1_dv_age_cons + hn3_dv_c30_ghs, data = aa %>% filter(missing==1))
summary(model)



hist( aa %>% filter(missing==0) %>% pull(hn4_dv_c30_ghs))
hist( aa %>% filter(missing==1) %>% pull(hn4_dv_c30_ghs))


saveRDS(imp_all, paste0(path, "data/imputed_simulated.rds"))
