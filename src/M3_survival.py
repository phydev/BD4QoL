...
df = preprocess(data = df, features = variable_list, target = "hn4_dv_status")
bs = preprocess(data = bs, features = bs_variables, target = "hn4_dv_status")
...
for bootstrap in range(0, n_bootstraps):
    ...
    y_train = bs_n.loc[:, "hn4_dv_status"]

    # bootstrap out-of-bag samples
    X_oob = oob.loc[:, covariates]
    y_oob = oob.loc[:, "hn4_dv_status"]
